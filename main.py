...
def test_and_cast(key, value):
    integers = ['num_ways', 'num_ways_training', 'num_ways_inference', 'num_shots', 'num_shots_training',
                ...
                'em_compression_nsup','gpu','sample_num']
    floats = ['learning_rate', 'norm_weight', 'fixsup_weight', 'sharpening_strength', 'regularization', 'dropout_rate', 
               'SGDmomentum','SGDweight_decay','pcl_weight','mixup_decision', 'cutmix_decision']
    strings = ['block_architecture','block_interface','sharpening_activation', 'log_dir', 'representation', 'log',
               ...
               'retrain_act','nudging_act','em_compression','train_subset','test_subset', 'incremental_mode']
...
submit_parent_parser.add_argument('--null', '-n', action='store_true',help='Use this option for dummy submission with output.')
submit_parent_parser.add_argument('--limited', '-l', action='store_true',help='Run a limited number of jobs in parallel.')
submit_parent_parser.add_argument('--queue', '-q', type=str, choices=['prod.short', 'prod.med', 'prod.long'],help='Specify the queue to submit the jobs to.')
...
# Experiment parser
experiment_parser = subparsers.add_parser('experiment', parents=[],help='Finish PCM experiments that have been run on the experimental platform.')
experiment_parser.add_argument('logdir', type=str,help='Specify the logging directory where the memory outputs are stored.')
experiment_parser.add_argument('--verify', '-y', action='store_true',help='Simulation with lossless external memory.')
experiment_parser.set_defaults(which='experiment')

# Simulation parser
simulation_parser = subparsers.add_parser('simulation', parents=[log_parent_parser, verbose_parent_parser],help='Start a simulation of the model.',)
simulation_parser.add_argument('-p', '--parameter', action='append', nargs=2, metavar=('key', 'value'),help='Specify parameters with key-value pairs. Chainable.')
simulation_parser.set_defaults(which='simulation')


# Cleanup parser
cleanup_parser = subparsers.add_parser('cleanup',parents=[verbose_parent_parser],help='Clean up a logging directory structure.',)
cleanup_parser.add_argument('function', type=str, choices=['unfinished', 'checkpoints'],help='Choose what to clean up. "unfinished" deletes unfinished runs, "checkpoint" deletes trainable parameter backup files.')
cleanup_parser.add_argument('logdir', type=str, help='Specify the root to clean up.')
cleanup_parser.set_defaults(which='cleanup')

# Evaluation parser
evaluation_parser = subparsers.add_parser('evaluation',parents=[verbose_parent_parser],help='Use after simulations have been run.',)
evaluation_parser.add_argument('function', type=str, choices=['gather', 'test', 'copy_best'],help='Use "gather" to collect all results in a single file.')
evaluation_parser.add_argument('logdir', type=str, help='Specify the root to collect the results from.')
evaluation_parser.add_argument('-b', '--best', action='store_true',help='')
evaluation_parser.set_defaults(which='evaluation')

# Submit parser
submit_parser = subparsers.add_parser('submit',parents=[verbose_parent_parser],help='Use to dispatch multiple simulations specified in "submitter.py".',)
...
submit_simulation_parser = \
    submit_subparsers.add_parser('simulations',parents=[submit_parent_parser, verbose_parent_parser],help='Submit multiple simulations to the cluster.',)
submit_simulation_parser.add_argument('--indices', '-i', type=int, nargs='+',help='Specify indices of "parameter_list" that should be submitted. Use spaces to separate indices.')
submit_simulation_parser.add_argument('--shuffle', '-s', action='store_true',help='In case the submission order should be shuffled.')
submit_simulation_parser.set_defaults(which='submit_simulations')

# Experiment submitter
submit_simulation_parser = \
    submit_subparsers.add_parser('experiments',help='Finish multiple experiments.',parents=[submit_parent_parser, verbose_parent_parser])
submit_simulation_parser.add_argument('root', type=str,help='Specify the root of the experiments.',)
submit_simulation_parser.add_argument('--verify', '-y', action='store_true',help='Simulation with lossless external memory.')
